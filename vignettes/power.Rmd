...
In this case $\delta = 10$ corresponds to $p_a=`r barea`$
...
With $\delta = 10$ as above we have $p_a=`r barea`$

So the $P(Type 2 error)=\beta$ is:

```{r}
pa <- barea
pbinom(24,36,pa)-pbinom(11,36,pa)
```
...
Now, with $\alpha = `r alpha`$ and $\delta = 10$ the parametric test gives:
